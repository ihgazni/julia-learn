...
;;;;
;;;;
(define (tst-strdot op) (string "." op))
;;;; "." + op
(define (tst-symbol op) (symbol (tst-strdot op)))
;;;; 单引号' 与quote是等价的 quote之后的直接当成数据
(define (tst-map ops) (map tst-symbol ops))
;;;; (tst-map '(777 899))
(define (tst-append ops)(append! ops (tst-map ops)))
;;;;https://www.gnu.org/software/guile/manual/html_node/Append_002fReverse.html
;;;;The procedure variants with an ! directly modify the pairs which form the list, whereas the other procedures create new pairs. This is why you should be careful when using the side-effecting variants.
(define (add-dots ops) (tst-append ops))
;;;;
;;;;
